...
Common Ground is a mobile-first social World for [Decentraland](https://decentraland.org), built for the [Friendzone Mobile Buildathon](https://dorahacks.io/hackathon/friendzone) (Aug–Sep 2026). You tap three interest "sparks", the plaza seats you at the campfire table whose past visitors share the most with you, and one tapped answer makes the World name a real person who answered the same way, whether they are standing next to you or visited last week.
...
2. **The World seats you.** Four tables (Ember, Driftwood, Lantern, North Star). You are sent to the one whose recent answerers share the most sparks with you.
3. **Answer at the stones.** Tap your table and story stones rise with an icebreaker written for your sparks and its three or four answers. Tap one.
...
- **Named cross-time matching.** A four-rung match ladder (same answer and shared spark, shared spark, same answer, anyone before you) means the reveal never dead-ends, and the wording is honest to the rung. It fires at concurrency one because the World keeps the last 40 answers on every board and matches against all of them.
- **A world made of its own data.** The most recent answers on each board stand as lanterns in an arc behind it, lit in the author's spark colour. The fire's size is the count of every answer ever given. Nothing is decorative.
...
- Type is scaled up against the desktop design on phones and the spark grid reflows, so the same screens hold in the landscape view the mobile app presents a World in, and in portrait if a client ever offers it.
